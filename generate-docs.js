    ...
    sections: [
        {
            ...
            children: [
                ...
                createParagraph("2. Introduction to SanjeevaniBharat ............................................................................. Page 4"),
                ...
                createParagraph("SanjeevaniBharat was born out of the necessity to bridge this gap. Designed as a high-fidelity, doctor-centric application, SanjeevaniBharat provides a seamless interface for medical professionals to manage their patient database while automating the critical task of medication monitoring. By leveraging the ubiquity of WhatsApp, the platform ensures that reminders are delivered through a channel that patients already use and trust daily."),
                createParagraph("This document provides an exhaustive overview of the SanjeevaniBharat ecosystem, detailing its features, technical architecture, and the strategic value it brings to both clinical practices and patient outcomes. From its intuitive dashboard to its sophisticated backend automation, SanjeevaniBharat represents the next step in digital health assistants."),
                new Paragraph({ children: [new PageBreak()] }),

                // PAGE 4: INTRODUCTION
                createHeading("2. Introduction to SanjeevaniBharat"),
                createParagraph("SanjeevaniBharat is more than just a scheduling tool; it is a full-featured clinical management assistant tailored for the modern practitioner. In a busy clinic, a doctor spends hours diagnosing and prescribing. However, once the patient leaves the clinic, the doctor's influence often wanes. SanjeevaniBharat extends the doctor's care into the patient's home, acting as a digital proxy that monitors health schedules in real-time."),
                createSubHeading("2.1 Why SanjeevaniBharat?"),
                createParagraph("Traditional methods of medication reminders—such as paper checklists or dedicated health apps—frequently fail because they require too much effort from the patient. Most dedicated apps are deleted within weeks, and paper lists are easily forgotten. SanjeevaniBharat solves this by placing the reminder exactly where the patient is: in their WhatsApp inbox."),
                ...
                createParagraph("Managing a diverse patient base requires a robust organization system. SanjeevaniBharat's Patient Management module is built to be both powerful and user-friendly. Each patient is treated as a unique entity with a comprehensive digital file."),
                ...
                createParagraph("The core engine of SanjeevaniBharat is its sophisticated scheduling algorithm. Prescribing a medicine is not just about naming the drug; it's about defining a precise rhythm. SanjeevaniBharat supports complex scheduling patterns to match any prescription."),
                ...
                createParagraph("SanjeevaniBharat doesn't just send a message and walk away. It operates on a 'Check-and-Verify' logic. Every minute, a background system (Node-Cron) scans the database for scheduled medicines that are due for a reminder."),
                ...
                createParagraph("The integration with Twilio's WhatsApp API is the backbone of SanjeevaniBharat's communication strategy. WhatsApp has over 2 billion active users and an open rate of over 90%, making it the most effective channel for healthcare reminders. Our implementation uses secure Webhooks to handle the flow of data back and forth."),
                ...
                createParagraph("SanjeevaniBharat is not a one-way broadcasting system; it is a conversation-based tool. Patients can reply to reminders to update the system. This 'Closed-Loop' communication is what sets SanjeevaniBharat apart from simple alarm apps."),
                ...
                createParagraph("SanjeevaniBharat is built on a modern, high-performance stack designed for reliability and speed."),
                ...
                createParagraph("Given the sensitivity of medical data, SanjeevaniBharat adheres to strict security standards. All API routes are protected by session tokens, and cross-origin requests are strictly controlled. Database migrations are managed via Prisma, ensuring a consistent and versioned schema."),
                new Paragraph({ children: [new PageBreak()] }),

                // PAGE 15: TROUBLESHOOTING AND SUPPORT
                createHeading("9. Troubleshooting and Support"),
                createParagraph("While SanjeevaniBharat is designed for 99.9% uptime, we provide comprehensive troubleshooting resources for clinic administrators."),
                ...
                createParagraph("SanjeevaniBharat is constantly evolving. Our roadmap includes AI-powered adherence predictions, a dedicated patient portal for viewing historical lab results, and integration with pharmacy delivery services. In conclusion, SanjeevaniBharat is more than a software product; it is a commitment to better health through better communication. We are proud to support healthcare heroes in their mission to provide excellent care."),
                new Paragraph({
                    children: [
                        new TextRun({
...
Packer.toBuffer(doc).then((buffer) => {
    fs.writeFileSync("SanjeevaniBharat_Documentation.docx", buffer);
    console.log("Document created successfully: SanjeevaniBharat_Documentation.docx");
}).catch(err => {
